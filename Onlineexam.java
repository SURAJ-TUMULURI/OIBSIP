...
public class Onlineexam {
	...
	public Onlineexam(String username, String password) {
		...
		this.isLogged = false;
		this.timeLeft = 10; // in minutes
		this.questionCount = 10;
		this.userOptions = new int[questionCount];
		this.correctOptions = new int[questionCount];
		// initialize correct answers with random values (0 or 1)
		for (int i = 0; i < questionCount; i++) {
			correctOptions[i] = (int) Math.round(Math.random());
		}
	}
	...
	public void startExam() {
		...
		Scanner scanner = new Scanner(System.in);
		System.out.println("You have " + timeLeft + " minutes to complete the exam.");
		for (int i = 0; i < questionCount; i++) {
